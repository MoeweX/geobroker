...
class ZMQProcess_BrokerCommunicator extends ZMQProcess {
	 ...
	 */
	ZMQProcess_BrokerCommunicator(String identity, IDistributionLogic distributionLogic, List<BrokerInfo> otherBrokerInfos) {
		super(identity);
	 ...
	 */
	static String getBrokerCommunicatorAddress(String identifier) {
		return "inproc://" + identifier;
	}
	...
	protected List<Socket> bindAndConnectSockets(ZContext context) {
		...
		for (BrokerInfo brokerInfo : otherBrokerInfos) {
			logger.debug("Connecting to {}", brokerInfo.generateZMQtcpAddress());
			Socket dealer = context.createSocket(SocketType.DEALER);
			dealer.setIdentity((identity + "-" + i).getBytes());
			dealer.connect(brokerInfo.generateZMQtcpAddress());
